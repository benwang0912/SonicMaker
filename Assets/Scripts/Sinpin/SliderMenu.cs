...
public class SliderMenu : MonoBehaviour {
    ...
    public void changeBackground() {
        float x = HorizontalScrollBar.GetComponent<Scrollbar>().value;
        if (x <= ScrollSteps/2 && bg.sprite != Background[0]) {
            bg.sprite = Background[0];
        } else if (x > ScrollSteps / 2 && x <= ScrollSteps / 2 * 3 && bg.sprite != Background[1])
        {
            bg.sprite = Background[1];
        }
        else if (x > ScrollSteps / 2 * 3 && x <= ScrollSteps / 2 * 5 && bg.sprite != Background[2])
        {
            bg.sprite = Background[2];
        }
        else if (x > ScrollSteps / 2 * 5 && bg.sprite != Background[3])
        {
            bg.sprite = Background[3];
        }
